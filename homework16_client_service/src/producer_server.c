...
struct pollfd *tcp_pfds = NULL;
struct pollfd *udp_pfds = NULL;
...
void shutdown_server(void)
{
	...
	pthread_cancel(tcp_listener_tid);
	pthread_cancel(udp_listener_tid);
	...
	unlink(SERVER_TCP_CLIENTS_Q_NAME);
    unlink(SERVER_UDP_CLIENTS_Q_NAME);
	unlink(SERVER_TCP_ACK_CLIENTS_Q_NAME);
    unlink(SERVER_UDP_ACK_CLIENTS_Q_NAME);
...
/* Thread, polling, until message from TCP client arrives */
void *_tcp_listener(void *args)
{
	mqd_t tcp_fds_q;
	mqd_t tcp_clients_q;
	mqd_t tcp_ack_clients_q;
	struct mq_attr tcp_msgs_attr;
	struct mq_attr tcp_acks_attr;
	struct pollfd *tmp_pfd = NULL;
	int fd = 0;
	...
	tcp_msgs_attr.mq_maxmsg = SERVER_Q_MAXMSG;
	tcp_msgs_attr.mq_msgsize = SERVER_CLIENTS_Q_MSGSIZE;
	tcp_clients_q = mq_open(SERVER_TCP_CLIENTS_Q_NAME, O_CREAT | O_RDWR, 0666, 
								&tcp_msgs_attr);
    if (tcp_clients_q == -1)
    {
	...
	tcp_acks_attr.mq_maxmsg = SERVER_Q_MAXMSG;
	tcp_acks_attr.mq_msgsize = SERVER_CLIENTS_Q_MSGSIZE;
	tcp_ack_clients_q = mq_open(SERVER_TCP_ACK_CLIENTS_Q_NAME, O_CREAT | O_RDWR, 0666, 
								&tcp_acks_attr);
    if (tcp_ack_clients_q == -1)
    {
		printf("tcp_fds_q mq_open: %s (%d)\n", strerror(errno), errno);
        exit(EXIT_FAILURE);
    }

	tcp_pfds = calloc(pfds_size, sizeof(struct pollfd));
	if (tcp_pfds == NULL)
	{
		perror("udp listener malloc");
		exit(EXIT_FAILURE);
	}

	tcp_pfds[0].fd = tcp_fds_q;
	tcp_pfds[0].events = POLLIN;
	tcp_pfds[1].fd = tcp_ack_clients_q;
	tcp_pfds[1].events = POLLIN;
	...
	while (1)
	{
		ret = poll(tcp_pfds, pfds_size, -1);
		switch (ret)
		{
		case 0:
			break;
		case -1:
			perror("tcp listener poll");
			exit(EXIT_FAILURE);
		default:
		{
			if (tcp_pfds[0].revents & POLLIN)
			{
				/* Wait for fd from main thread */
				if (mq_receive(tcp_fds_q, &fd, SERVER_TCP_Q_MSGSIZE, NULL) 
					== -1)
				{
					printf("tcp listener mq_receive: %s (%d)\n", strerror(errno), errno);
					continue;
				}
				...
				tmp_pfd = reallocarray(tcp_pfds, pfds_size, sizeof(struct pollfd));
				if (tmp_pfd == NULL)
				{
					perror("tcp listener realloc");
					exit(EXIT_FAILURE);
				}
				tcp_pfds = tmp_pfd;
				
				tcp_pfds[pfds_size-1].fd = fd;
				tcp_pfds[pfds_size-1].events = POLLIN;

				tcp_pfds[0].revents = 0;
			}
			if (tcp_pfds[1].revents & POLLIN)
			{
				if (mq_receive(tcp_pfds[1].fd, &fd, SERVER_TCP_Q_MSGSIZE, NULL) 
					== -1)
				{
					printf("tcp listener mq_receive: %s (%d)\n", strerror(errno), errno);
					continue;
				...
				for (index = 2; index < pfds_size; ++index)
				{
					if (tcp_pfds[index].fd == fd)
					{
						tcp_pfds[index].events = POLLIN;
						break;
					}
				}

				tcp_pfds[1].revents = 0;
			}
			for (index = 2; index < pfds_size; ++index)
			{
				if (tcp_pfds[index].revents & POLLIN)
				{
					while (mq_send(tcp_clients_q, &tcp_pfds[index].fd, SERVER_CLIENTS_Q_MSGSIZE, 
						NULL) == -1)
					{
						if (errno != EAGAIN)
						{
...
/* Thread, polling, until message from UDP client arrives */
void *_udp_listener(void *args)
{
	mqd_t udp_endpoints_q;
	mqd_t udp_clients_q;
	mqd_t udp_ack_clients_q;
	struct mq_attr udp_msgs_attr;
	struct mq_attr udp_acks_attr;
	struct pollfd *tmp_pfd = NULL;
	int fd = 0;
	struct sockaddr_in server;
	struct sockaddr_in endp;
	...
	udp_msgs_attr.mq_maxmsg = SERVER_Q_MAXMSG;
	udp_msgs_attr.mq_msgsize = SERVER_CLIENTS_Q_MSGSIZE;
	udp_clients_q = mq_open(SERVER_UDP_CLIENTS_Q_NAME, O_CREAT | O_RDWR, 0666, 
								&udp_msgs_attr);
    if (udp_clients_q == -1)
    {
	...
	udp_acks_attr.mq_maxmsg = SERVER_Q_MAXMSG;
	udp_acks_attr.mq_msgsize = SERVER_CLIENTS_Q_MSGSIZE;
	udp_ack_clients_q = mq_open(SERVER_UDP_ACK_CLIENTS_Q_NAME, O_CREAT | O_RDWR, 0666, 
								&udp_acks_attr);
    if (udp_ack_clients_q == -1)
    {
		printf("udp_fds_q mq_open: %s (%d)\n", strerror(errno), errno);
        exit(EXIT_FAILURE);
    }

	udp_pfds = malloc(sizeof(struct pollfd)*pfds_size);
	if (udp_pfds == NULL)
	{
		perror("udp listener malloc");
		exit(EXIT_FAILURE);
	...
	udp_pfds[0].fd = udp_endpoints_q;
	udp_pfds[0].events = POLLIN;
	udp_pfds[1].fd = udp_ack_clients_q;
	udp_pfds[1].events = POLLIN;
	...
	while (1)
	{
		ret = poll(udp_pfds, pfds_size, -1);
		switch (ret)
		{
		case 0:
			break;
		case -1:
			perror("udp listener poll");
			exit(EXIT_FAILURE);
		default:
		{
			if (udp_pfds[0].revents & POLLIN)
			{
				/* Wait for endpoint from main thread */
				if (mq_receive(udp_endpoints_q, &endp, SERVER_UDP_Q_MSGSIZE, NULL) 
					== -1)
				{
					printf("udp listener mq_receive: %s (%d)\n", strerror(errno), errno);
					continue;
				}
				...
				tmp_pfd = realloc(udp_pfds, sizeof(struct pollfd)*pfds_size);
				if (tmp_pfd == NULL)
				{
					perror("udp listener realloc");
					exit(EXIT_FAILURE);
				}
				udp_pfds = tmp_pfd;
				...
				if (connect(fd, &endp, sizeof(endp)) != 0)
				{
					perror("udp listener connect");
					exit(EXIT_FAILURE);
				}

				/* Send a message, so that client could receive */
				strncpy(msg, SERVER_MSG, CLIENT_MSG_SIZE);
				if (send(fd, msg, SERVER_MSG_SIZE, NULL) 
					== -1)
				{
				...
				udp_pfds[pfds_size-1].fd = fd;
				udp_pfds[pfds_size-1].events = POLLIN;

				udp_pfds[0].revents = 0;
			}
			if (udp_pfds[1].revents & POLLIN)
			{
				if (mq_receive(udp_pfds[1].fd, &fd, SERVER_CLIENTS_Q_MSGSIZE, NULL) 
					== -1)
				{
					printf("udp listener mq_receive: %s (%d)\n", strerror(errno), errno);
					continue;
				...
				for (index = 2; index < pfds_size; ++index)
				{
					if (udp_pfds[index].fd == fd)
					{
						udp_pfds[index].events = POLLIN;
						break;
					}
				}

				udp_pfds[1].revents = 0;
			}
			for (index = 2; index < pfds_size; ++index)
			{
				if (udp_pfds[index].revents & POLLIN)
				{
					while (mq_send(udp_clients_q, &udp_pfds[index].fd, SERVER_CLIENTS_Q_MSGSIZE, 
						NULL) == -1)
					{
						if (errno != EAGAIN)
						{
...
void *_tcp_server_thread(void *args)
{
    ...
    */
	...
	/* Open 'tcp_clients_q' & 'tcp_ack_clients_q' */
	while ((tcp_clients_q = mq_open(SERVER_TCP_CLIENTS_Q_NAME, O_RDONLY)) == -1)
	{
		printf("tcp mq_open: %s (%d)\n", strerror(errno), errno);
		exit(1);
	}
	while ((tcp_ack_clients_q = mq_open(SERVER_TCP_ACK_CLIENTS_Q_NAME, O_WRONLY)) == -1)
	{
		printf("tcp mq_open: %s (%d)\n", strerror(errno), errno);
	}
    ...
    {
		/* Wait for fd from main thread */
        if (mq_receive(tcp_clients_q, &fd, sizeof(int), NULL) == -1)
        {
        ...
        {
			printf("tcp processor send: %s (%d)\n", strerror(errno), errno);
			if (errno == ECONNRESET)
				exit(EXIT_FAILURE);
        }

		while (mq_send(tcp_ack_clients_q, &fd, SERVER_CLIENTS_Q_MSGSIZE, 
						NULL) == -1)
		{
			if (errno != EAGAIN)
			{
	...
	/* Close MQ fds */
    mq_close(tcp_clients_q);
	mq_close(tcp_ack_clients_q);
...
void *_udp_server_thread(void *args)
{
    ...
    */
	...
	/* Open 'tcp_fds_q' */
	while ((udp_clients_q = mq_open(SERVER_UDP_CLIENTS_Q_NAME, O_RDONLY)) == -1)
	{
		printf("udp mq_open: %s (%d)\n", strerror(errno), errno);
	}

	/* Open 'tcp_fds_q' */
	while ((udp_ack_clients_q = mq_open(SERVER_UDP_ACK_CLIENTS_Q_NAME, O_WRONLY)) == -1)
	{
		printf("udp mq_open: %s (%d)\n", strerror(errno), errno);
	}
    ...
    while(1)
    {
		/* Wait for fd from listener thread */
		if (mq_receive(udp_clients_q, &fd, SERVER_CLIENTS_Q_MSGSIZE, NULL) 
			== -1)
        {
		...
		if (send(fd, &msg, sizeof(msg), 0) == -1)
        {
			printf("udp processor send: %s (%d)\n", strerror(errno), errno);
			if (errno == ECONNRESET)
				exit(EXIT_FAILURE);
        }

		while (mq_send(udp_ack_clients_q, &fd, SERVER_CLIENTS_Q_MSGSIZE, 
						NULL) == -1)
		{
			if (errno != EAGAIN)
			{
	...
	/* Close MQ fd */
    mq_close(udp_clients_q);
	mq_close(udp_ack_clients_q);
...
int producer_server()
{
    ...
    */
	...
	pthread_create(&tcp_listener_tid, NULL, _tcp_listener, NULL);
	pthread_create(&udp_listener_tid, NULL, _udp_listener, NULL);

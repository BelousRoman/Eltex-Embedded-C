...
int server(void)
{
    ...
    */
	...
	/* Fill 'server' and 'client' with 0's */
    ...
    if (recvfrom(server_fd, msg, MSG_SIZE, 0, (struct sockaddr *)&client, &client_size) == -1)
    {
        printf("recvfrom: %s (%d)\n", strerror(errno), errno);
        return EXIT_FAILURE;
    }
    ...
    if (sendto(server_fd, msg, MSG_SIZE, 0, (struct sockaddr *)&client, sizeof(client)) == -1)
    {
        printf("sendto: %s (%d)\n", strerror(errno), errno);
        return EXIT_FAILURE;
    }

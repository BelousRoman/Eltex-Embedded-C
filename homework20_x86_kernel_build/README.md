...
Build was performed on the Linux Kernel v4.15 using working config file from /boot/ directory.

To build a kernel for x86 system the following steps should be made:
1)  Hop onto required kernel version by calling 'git checkout v{your kernel version}'. 
    The version usually specified in the config file's name, i.e.:
...
Logs during build is provided in 'logs' directory with warning messages, received by particular commands.
